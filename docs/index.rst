...
.. tip::
    ...
    These placeholders can also be used inside table cells, and several placeholders can be combined
    in a single step argument:
    ...
    steps (a row cannot reference another row of the same table, and ``<lastId>`` in a creation table
    refers to the last object created before that step). They always use the canonical ``<...>`` syntax,
    even when the ``#[Transform]`` patterns have been re-worded.

.. warning::

    These placeholders only see objects persisted **through Foundry**: ``Given`` steps, stories and
    ``@withFixture`` fixtures (including objects created in ``afterPersist`` hooks). An object created
    by the application under test (e.g. by the endpoint exercised in a ``When`` step) never updates
    ``<lastId>``: the placeholder would silently resolve to the id of the last object created by
    Foundry. To assert on ids generated by the application itself, capture them from the response in
    your own context.
