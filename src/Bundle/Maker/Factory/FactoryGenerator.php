...
final class FactoryGenerator
{
    ...
    public function __construct(private ManagerRegistry $managerRegistry, private KernelInterface $kernel, private \Traversable $defaultPropertiesGuessers, private FactoryFinder $factoryFinder)
    {
    }
    ...
    public function generateFactory(SymfonyStyle $io, MakeFactoryQuery $makeFactoryQuery): string
    {
        ...
        if (!$this->factoryFinder->classHasFactory($class)) {
            $this->factoryFinder->addFactoryForClass($factoryClass, $class);
        }
