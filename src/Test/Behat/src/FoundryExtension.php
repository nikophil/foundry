    ...
    public function configure(ArrayNodeDefinition $builder): void
    {
        $builder
            ->children()
                ...
                ->end()
                ->arrayNode('steps')
                    ->info('Override built-in step definitions: map a canonical pattern to your own wording (keep the same placeholders/capture groups), or to false to disable the step.')
                    ->normalizeKeys(false)
                    ->scalarPrototype()
                        ->validate()
                            ->ifTrue(static fn(mixed $v): bool => !\is_string($v) && false !== $v)
                            ->thenInvalid('A step override must be a wording string, or false to disable the step.')
                        ->end()
                    ->end()
                ->end()
