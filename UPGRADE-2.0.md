...
All you have to do is to upgrade to the last 1.x version, activate the deprecation helper, make the tests run, and
...
- `sequence()` and `createSequence()` do not accept `callable` as a parameter anymore (how do we leverage `__index` instead??)
- Return type  of `initialize()` will change from `self` to `static`

### Change factories' parent class

`Zenstruck\Foundry\ModelFactory` is now deprecated.
You should choose between `\Zenstruck\Foundry\Persistence\PersistentProxyObjectFactory` and `\Zenstruck\Foundry\Persistence\PersistentObjectFactory`.
Both act the same way, but the latter does not return a `Proxy` object.
...
- only entities (ORM) or documents (ODM) should use `PersistentProxyObjectFactory`
- you should only use `PersistentProxyObjectFactory` if you want to leverage "auto refresh" behavior
- all "non-persistable" objects could use `PersistentObjectFactory`, since `Proxy` is only useful for entities/documents
- final object cannot use `PersistentProxyObjectFactory`, but that should not be a problem, since entities/documents cannot be final objects

> [!WARNING]
> `PersistentProxyObjectFactory` should not be chosen to create `final` classes instances.
