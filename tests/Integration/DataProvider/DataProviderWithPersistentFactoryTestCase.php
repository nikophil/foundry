...
abstract class DataProviderWithPersistentFactoryTestCase extends KernelTestCase
{
    ...
    #[Test]
    #[DataProvider('dataProviderUsingRandomRanges')]
    public function assert_data_provider_throws_when_using_range_in_data_provider(?\Throwable $throwable): void
    {
        self::assertInstanceOf(\InvalidArgumentException::class, $throwable);
        self::assertSame('Using randomized "range" factory in data provider is not supported.', $throwable->getMessage());
    }

    public static function dataProviderUsingRandomRanges(): iterable
    {
        try {
            GenericEntityFactory::createRange(1, 2);
        } catch (\Throwable $e) {
        }
        ...
        try {
            GenericEntityFactory::new()->range(1, 2);
        } catch (\Throwable $e) {
        }
